...
class BinaryTree:
    ...
    def list(self):
        def _print_weights(node):
            if node is None:
                return []
            left_values = _print_weights(node.left)
            right_values = _print_weights(node.right)
            return left_values + [node.value] + right_values

        values = _print_weights(self.root)
        print(*values)
...
while True:
    ...
    if command[0] == "add":
        element = [int(x) for x in command[1:]]
        for i in element:
            tree.add_value(i)
        print("Ok")
        ...
        element = tree.get_min_node()
        print(element.value if element != None else "Склад пуст")
    elif command[0] == "max":
        element = tree.get_max_node()
        print(element.value if element != None else "Склад пуст")
    else:
        break